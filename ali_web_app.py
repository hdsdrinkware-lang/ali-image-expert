...
st.set_page_config(page_title="阿里主图专家 V4.0", page_icon="🖼️")
...
st.title("🖼️ 阿里主图裁切专家 V4.0")
st.info("规格：1000x1000像素 | 比例 1:1 | 单张 ≤ 5MB | 支持直接粘贴图片")
pasted_result = paste_image_button("📋 粘贴图片 (Clipboard)")
uploaded_file = st.file_uploader("或上传图片", type=["jpg", "jpeg", "png", "webp"])
target_img = None
...
if target_img:
    ...
    c1, c2 = st.columns(2)
    if c1.button("✂️ 3x2 合集拆分"): results = split_ali_grid(target_img)
    elif c2.button("🔍 单图生成细节"): results = get_ali_images(target_img)
    else: results = None
    if results:
        ...
        st.download_button("📥 一键下载全部 6 张主图 (ZIP)", zip_buffer.getvalue(), "ali_images.zip", "application/zip")
